...
const STORAGE_KEY = "automax_showroom_v2";
...
/* ============================================================
   ...
   ============================================================ */
const db = {
  obtener: () => JSON.parse(localStorage.getItem(STORAGE_KEY)) || [],
  guardar: (data) => localStorage.setItem(STORAGE_KEY, JSON.stringify(data)),
  agregar(v) { const d = this.obtener(); d.push(v); this.guardar(d); },
  actualizar(orig, v) {
    const d = this.obtener();
    const i = d.findIndex(x => x.codigo === orig);
    if (i !== -1) { d[i] = v; this.guardar(d); }
  },
  eliminar(cod) { this.guardar(this.obtener().filter(v => v.codigo !== cod)); },
  existe(cod, exc = "") { return this.obtener().some(v => v.codigo.toLowerCase() === cod.toLowerCase() && v.codigo !== exc); }
};
...
D.fleetGrid.addEventListener("click", e => {
  const btn = e.target.closest("[data-action]");
  if (!btn) return;
  const cod = btn.dataset.cod;
  if (btn.dataset.action === "edit")   cargarParaEditar(cod);
  if (btn.dataset.action === "delete") abrirModal(cod);
});

D.buscador.addEventListener("input", e => renderFleet(e.target.value));
...
function validar() {
  ...
  else if (db.existe(cod, codOrig))      { markField("codigo", "Código ya registrado."); ok = false; }
...
/* ============================================================
   CARGAR PARA EDITAR
   ============================================================ */
function cargarParaEditar(cod) {
  const v = db.obtener().find(x => x.codigo === cod);
  if (!v) return;
